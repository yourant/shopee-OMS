  ...
  export default {
    ...
    data() {
      return {
        // 权限
        permission: {
          add: 'operation.manager.user.add', // 增
          delete: 'operation.manager.user.delete', // 删
          enable: 'operation.manager.user.enable', // 可用
          editRole: 'operation.manager.user.edit-role', // 修改权限
          account: 'operation.manager.user.edit-user-account' // 账号授权
        },
    ...
    methods: {
      ...
      /* 账号授权 */
      editAccount(d) {
        operableAccount({ user_id: d.id }).then(res => {
          const arr = []
