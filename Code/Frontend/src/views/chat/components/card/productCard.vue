<template>
  <div class="product_card" v-loading="loading" element-loading-background="rgba(255, 255, 255, 1)">
    ...
    <div style="flex-grow: 1;">
      ...
      <div class="small-text mt-2">产品ID: {{ productData.serial_number }}</div>
      <div v-if="productData.models && productData.models.length" class="small-text mt-2">子产品ID:{{ ` ${productData.models[0]['serial_number']}` }}</div>
    </div>
    <div class="right-box">
  ...
  export default {
    ...
    methods: {
      ...
      formatShowData(obj) {
        ...
        let mailUrl = this.siteCode === 'tw' ? 'https://xiapi.xiapibuy.com' : this.$store.state.platform.info.web_url
